import { setTransform, equalizePoints, decodeImage, toTransformString } from '@/util/util';
...
import { AnimationProps } from '@/util/animations';
...
class Opener {
  ...
  _cropContainer1?: HTMLDivElement | null;
  _cropContainer2?: HTMLElement | null;
  _thumbBounds?: Bounds;
  _animateZoom: false | undefined | boolean;
  ...
  _start() {
    if (this.isOpening && this._useAnimation && this._placeholder && this._placeholder.tagName === 'IMG') {
      ...
      new Promise((resolve) => {
        let decoded = false;
        let isDelaying = true;
        decodeImage(/** @type {HTMLImageElement} */ this._placeholder as HTMLImageElement).finally(() => {
          decoded = true;
          if (!isDelaying) {
            resolve(true);
          }
   ...
   */
  _setClosedStateZoomPan(animate?: boolean) {
   ...
   */
  _animateTo(target: HTMLElement, prop: 'transform' | 'opacity', propValue: string) {
    if (!this._duration) {
