import { createElement, equalizePoints, pointsEqual, clamp } from '@/util/util';
...
import { ZoomLevelOption } from '@/slide/zoom-level';
...
import { ItemHolder } from '@/main-scroll';
 ...
 */
import { AugmentedEvent } from '@/core/eventable';
...
class PhotoSwipe extends PhotoSwipeBase {
  ...
  _initialItemData: SlideData;
  _initialThumbBounds?: Bounds;
   ...
   */
  zoomTo(...args: Parameters<Slide['zoomTo']>) {
  ...
  updateSize(force?: boolean) {
    ...
    const newViewportSize = getViewportSize(this.options as Partial<PreparedPhotoSwipeOptions>, this);
  ...
  applyBgOpacity(opacity: number) {
    ...
    if (this.bg) {
      this.bg.style.opacity = String(this.bgOpacity * Number(this.options?.bgOpacity));
    }
  }
  ...
  _createMainStructure() {
    ...
    // Background is added as a separate element, as animating opacity is faster than animating rgba()
   ...
   */
  canLoop(): boolean {
    return Boolean(this.options?.loop && this.getNumItems() > 2);
  }
   ...
   */
  _prepareOptions(options: PhotoSwipeOptions): PreparedPhotoSwipeOptions {
