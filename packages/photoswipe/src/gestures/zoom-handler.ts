...
import { equalizePoints, getDistanceBetween, clamp, pointsEqual } from '@/util/util';
 ...
 * @returns {Point}
 */
function getZoomPointsCenter(p: Point, p1: Point, p2: Point) {
  p.x = (p1.x + p2.x) / 2;
  p.y = (p1.y + p2.y) / 2;
...
class ZoomHandler {
   ...
   * @returns {number}
   */
  _calculatePanForZoomLevel(axis: Axis, currZoomLevel: number) {
